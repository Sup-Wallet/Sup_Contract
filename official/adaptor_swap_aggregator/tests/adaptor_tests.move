#[test_only]
module adaptor_swap_aggregator::adaptor_tests;

use adaptor_swap_aggregator::adaptor::{Self, SwapAggregatorAdaptor};
...
#[test]
fun test_aggregator_swap_round_trip() {
    ...
    scenario.next_tx(ALICE);
    {
        ...
        wallet::grant_service_coin<SwapAggregatorAdaptor, SUI>(&mut wallet, scenario.ctx());
    ...
    {
        let mut wallet: Wallet = scenario.take_shared();
        let (coin_in, swap_witness) = adaptor::begin_swap<SUI, SUI>(
            &mut wallet,
            1_000,
            900,
            scenario.ctx(),
        ...
        let coin_out = coin::mint_for_testing<SUI>(950, scenario.ctx());
        adaptor::finish_swap<SUI, SUI>(&mut wallet, swap_witness, coin_out);
        ts::return_shared(wallet);
    };
...
fun test_aggregator_swap_slippage_aborts() {
    let mut scenario = ts::begin(ALICE);
...
fun test_aggregator_swap_unauthorized_aborts() {
    let mut scenario = ts::begin(ALICE);
