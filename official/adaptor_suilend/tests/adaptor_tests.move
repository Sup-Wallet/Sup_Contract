#[test_only]
module adaptor_suilend::adaptor_tests;

use adaptor_suilend::adaptor::{Self, SuilendAdaptor};
...
#[test]
fun test_suilend_deposit_round_trip() {
    ...
    scenario.next_tx(ALICE);
    {
        ...
        wallet::grant_service_coin<SuilendAdaptor, SUI>(&mut wallet, scenario.ctx());
    ...
    scenario.next_tx(ALICE);
    {
        ...
        let ctoken_out = coin::mint_for_testing<SUI>(950, scenario.ctx());
        adaptor::finish_deposit<SUI, SUI>(&mut wallet, witness, ctoken_out);
        ts::return_shared(wallet);
    };
...
fun test_suilend_deposit_min_out_aborts() {
    let mut scenario = ts::begin(ALICE);
...
fun test_suilend_unauthorized_aborts() {
    let mut scenario = ts::begin(ALICE);
